...
function sendUnreachableResponse(ipHeader) do
  warn("sendUnreachableResponse not implemented yet")
end
...
function signalHandler(ipHeader, ...) do
  ...
  if not conn then
    ...
    sendUnreachableResponse(ipHeader)
    return
  end
...
function ip.isAddrInSubnet(subnet, addr)
  return subnet == addr or addr:find("^"..subnet:gsub("%.", "%%."))
end

function ip.isAddrMatch(a1, a2)
  return ip.isAddrInSubnet(a1, a2) or ip.isAddrInSubnet(a2, a1)
end
...
function ip.getListener(addr, port)
  ...
  for i,c in pairs(ip.listeners) do
    if c.port == port and ip.isAddrMatch(addr, c.addr) then
      return c, i
    end
  end
end
...
function ip.getInterface(addr)
  for _,ifcfg in ipairs(ip.interfaces) do
    if ip.isAddrMatch(addr, ifcfg.addr) then
      return ifcfg
    end
  end
end
